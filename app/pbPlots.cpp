...
{
  double y_length, yPixelLength;

  y_length = yMax - yMin;
  yPixelLength = yPixelMax - yPixelMin;

  y = y - yMin;
  y = y * yPixelLength / y_length;
  y = yPixelLength - y;
  y = y + yPixelMin;
  return y;
}
...
void DrawTextUpwards(RGBABitmapImage *canvas, double x, double y, vector<wchar_t> *text, RGBA *color)
{
  ...
  buffer = CreateImage(GetTextWidth(text), GetTextHeight(), GetTransparent());
...
{
  double xMin, xMax, yMin, yMax, xLength, y_length, i, x, y, xPrev, yPrev, px, py, pxPrev, pyPrev, originX, originY, p, l, plot;
  Rectangle *boundaries;
...
void AppendBitsToBytesRight(vector<double> *bytes, NumberReference *nextbit,
double data, double length) {
